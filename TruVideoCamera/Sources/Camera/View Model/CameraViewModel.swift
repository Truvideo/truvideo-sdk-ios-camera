...
public class CameraViewModel: ObservableObject {
    ...
    @Published private(set) var authenticationStatus: DataLoadStatus = .initial
    ...
    /// A count of each clips taking.
    @Published private(set) var countClips: Int = 0
    ...
    public init(
        ...
        tokenProvider: TokenProvider = BearerTokenProvider(storage: UserDefaultsStorage())
    ) {
    ...
    /// Handles the authentication and emits the status for the view.
    private func authentication() {
        Task { @MainActor in
            authenticationStatus = .loading
            
            guard let _ = await tokenProvider.retrieveToken() else {
    ...
    public func record() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now().advanced(by: DispatchTimeInterval.milliseconds(300))) {
