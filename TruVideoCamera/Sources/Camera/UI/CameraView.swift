...
public struct RecordingResult {
    ...
    public let clips: [TruVideoClip]

    /// The photos taken during the session.
    public let photos: [TruVideoPhoto]
}
...
public struct CameraView: View {
    ...
    public var body: some View {
        NavigationView {
            ZStack {
                if viewModel.authenticationStatus == .success {
                    Camera(dismiss: dismiss)
                } else {
                    Text("USER IS UNAUTHENTICATED")
                }   
            }
            ...
            .onChange(of: viewModel.recordStatus) { status in
                ...
                onComplete(RecordingResult(clips: viewModel.clips, photos: viewModel.photos))
            }
        }
    }
