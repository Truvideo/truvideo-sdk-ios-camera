...
public struct IconTheme: Sendable {
    ...
    ///   - size: The size of the icon in points. Defaults to `nil` for system default.
    public init(color: Color?, opacity: Double, size: Double?) {
        self.color = color
    ...
    ///   - opacity: Optional new opacity value
    ///   - size: Optional new size value
    /// - Returns: A new `IconTheme` instance with the specified properties updated
    public func copyWith(color: Color? = nil, opacity: Double? = nil, size: Double? = nil) -> IconTheme {
        IconTheme(color: color ?? self.color, opacity: opacity ?? self.opacity, size: size ?? self.size)
    }
}
