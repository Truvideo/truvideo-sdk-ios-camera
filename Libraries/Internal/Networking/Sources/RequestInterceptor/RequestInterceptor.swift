...
public protocol RequestRetrier {
    ...
    func retry(_ request: Request, for session: Session, failedWith error: Error) async -> RetryPolicy
}
...
extension RequestRetrier {
    ...
    ///   - session: The `Session` that produced the `Request`.
    ///   - error: The `Error` encountered while executing the `Request`.
    public func retry(_ request: Request, for session: Session, failedWith error: Error) async -> RetryPolicy {
        .doNotRetry
    }
}
