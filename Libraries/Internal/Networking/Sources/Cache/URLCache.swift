...
///
/// `URLCache` provides an interface for managing cached responses in a network session.
...
/// for future use.
public protocol URLCache: Sendable {
    ...
    ///
    /// - Parameter request: The `DataRequest` instance for which the cached response should be removed.
    func removeCachedResponse(for request: DataRequest)
}

extension URLCache where Self == InMemoryURLCache {
...
public struct InMemoryURLCache: URLCache, @unchecked Sendable {
    ...
    /// - Parameter request: The `DataRequest` instance for which the cached response is being requested.
    /// - Returns: A `URLCachedResponse` instance if a cached response exists, otherwise `nil`.
    public func cachedResponse(for request: DataRequest) -> URLCachedResponse? {
    ...
    ///   - response: The `URLCachedResponse` representing the response to be cached.
    ///   - request: The `DataRequest` for which the response is being cached.
    public func cache(_ response: URLCachedResponse, for request: DataRequest) {
    ...
    /// of using a cached response.
    ///
    /// - Parameter request: The `Request` instance for which the cached response should be removed.
    public func removeCachedResponse(for request: DataRequest) {
        if let cacheKey = request.cacheKey {
