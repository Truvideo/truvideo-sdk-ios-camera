...
///
/// Types conforming to `SessionDelegateProvider` are responsible for handling URL session events,
...
protocol SessionDelegateProvider: AnyObject, Sendable {
...
///
/// `SessionDelegate` acts as a bridge between `URLSession` and the `SessionDelegateProvider`,
