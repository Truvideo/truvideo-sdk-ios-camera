...
struct URLComponentsExtensionTests {
    ...
    @Test func testThatAsURLReturnsAValidURL() throws {
        // Given
        let url = URL(string: "https://httpbin.org/")!
        let sut = URLComponents(url: url, resolvingAgainstBaseURL: false)
        
        // When, Then
        #expect(try sut?.asURL() == url, "Expected url to be equals to \(url)")
    }
    
    @Test func testThatAsURLThrowsAnErrorOnInvalidURL() throws {
        // Given
